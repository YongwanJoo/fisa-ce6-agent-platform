...
def generate_answer(question: str, docs: list) -> str:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", (
            ...
            "만약 질문자가 제공한 상황(특히 웹훅을 통한 장애 감지)이 장애라고 판단된다면, 반드시 `send_discord_alert` 도구를 사용하세요.\n"
            "이때, 도구의 `message` 인자에는 단순히 알람 내용을 복사하는 것이 아니라, 반드시 위의 **장애 보고 형식(분석, 안전 체크, 빠른 해결 가이드)**을 모두 포함하여 아주 상세하게 작성해야 합니다.\n"
            "사용자가 '장애 분석해줘', '해결책 알려줘'라고 하지 않아도, 장애 감지 시에는 능동적으로 해결책을 포함한 리포트를 작성하는 것이 당신의 핵심 임무입니다."
        )),
        ("human", "질문: {question}\n\n참고 문서:\n{context}"),
    ])
